...
namespace TemperatureCharacteristics.Services
{
    public class DataService : IJsonDataService
    {
        ...
        //JSON読み込みメソッド
        //*************************************************
        public (ObservableCollection<PresetItemBase>, string, string) LoadItems<T>(string? filePath = null) where T : PresetItemBase
        {
            ...
            try
            {
                if (string.IsNullOrEmpty(filePath))
                {
                    var openFileDialog = new OpenFileDialog
                    {
                        ...
                        InitialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments)     //初期フォルダ
                    };
        ...
        //JSON保存メソッド
        //*************************************************
        public (bool, string, string) SaveItems<T>(string filePath = null, ObservableCollection<T> items = null) where T : PresetItemBase
        {
            string log = string.Empty;
            string selectedPath = filePath;
            try
            {
                ...
                if (string.IsNullOrEmpty(filePath))
                {
                    ...
                    var saveFileDialog = new SaveFileDialog
                    {
                        Filter = "JSON Files (*.json)|*.json|All Files (*.*)|*.*",
                        DefaultExt = "json",
                        FileName = "usersweepconfigs.json",
                        Title = "保存先を選択してください",
                        InitialDirectory = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments)     //初期フォルダ
                    };
