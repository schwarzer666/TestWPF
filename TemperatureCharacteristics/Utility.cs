...
namespace UTility
{
    ...
    public class UT
    {
        ...
        {
            return Path.Combine(GetTempDataFolder(), $"_DraftData_{timestamp}.csv");
        }
        //*************************************************
        //アクセス：public
        //戻り値：<string> 中間データ保存パス
        ...
        public string GetTabTempFilePath(string tabName, string baseTimestamp)
        {
            ...
            return Path.Combine(GetTempDataFolder(), $"_DraftData_{tabName}_{baseTimestamp}_{counter}.csv");
        }
        //*************************************************
        //アクセス：public
