...
{
    public class Sweep : IDeviceCombinable
    {
        ...
        private readonly SweepTabWindow sweepTab;   //フィールド変数sweepTab
        ...
        private Sweep()
        {
            ...
            sweepTab = SweepTabWindow.Instance;     //インスタンス生成(=初期化)を別クラス内で実行
    ...
    //    string DMMTrigSrc,
    //    string[] PGset)>> getMeasureOnTabData)
        public List<Device> CombineDeviceData(
            List<(bool IsChecked, string UsbId, string InstName, string Identifier)> meas_inst,
