...
{
    public class Delay : IDeviceCombinable
    {
        ...
        private readonly DelayTabWindow delayTab;   //フィールド変数DelayTab
        ...
        private Delay()
        {
            ...
            delayTab = DelayTabWindow.Instance;     //インスタンス生成(=初期化)を別クラス内で実行
    ...
    //    string[] Detrelset,
    //    string[] PGset,
    //    string[] Delayset)>> getMeasureOnTabData)
        public List<Device> CombineDeviceData(
            List<(bool IsChecked, string UsbId, string InstName, string Identifier)> meas_inst,
