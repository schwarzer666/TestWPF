...
namespace VIAction
{
    public class VI: IDeviceCombinable
    {
        ...
        private readonly VITabWindow viTab;         //フィールド変数VITab
        ...
        private VI()
        {
            ...
            viTab = VITabWindow.Instance;     //インスタンス生成(=初期化)を別クラス内で実行
    ...
    //    string DMMTrigSrc)>> getMeasureOnTabData)
        public List<Device> CombineDeviceData(
                                            List<(bool IsChecked, string UsbId, string InstName, string Identifier)> meas_inst,
                                            object getMeasureOnTabDataObj)
        {
            ...
            //オブジェクトを定義
            //*************************************
            var tabData = getMeasureOnTabDataObj as
                IEnumerable<(
                    string TabName, bool MeasureOn,
                    string[] Source1set, string[] Source2set, string[] Source3set, string[] Source4set,
                    string[] VIset, string[] Constset, string[] Detrelset,
                    bool[] DMMDisp,
                    string[] DMM1set, string[] DMM2set, string[] DMM3set, string[] DMM4set,
                    string DMMTrigSrc)>;
                                            ...
                                            List<Device>? preCombinedDevices = null)
        {
            ...
            try
            {
                ...
                //*********************
                deviceList = preCombinedDevices ?? CombineDeviceData(meas_inst, viTab.GetMeasureOnTabData);
                if (!deviceList.Any())
                {
                ...
                //測定Tab名を取得
                //*********************
                string TabName = viTab.CheckedTabNamesText.Trim();
